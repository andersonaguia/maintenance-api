...
export class PreventiveController {
  ...
  @Post('/create')
  async create(
    @Body() preventiveData: CreatePreventiveDto,
    @Request() req: any,
  ): Promise<DefaultResponseDto> {
    try {
      const result = await this.preventiveService.create(preventiveData, req);
      return result;
    } catch (error) {
      if (error.code == 404) {
  ...
  async findAll(): Promise<PreventiveEntity[]> {
    try {
      const result = await this.preventiveService.findAll();
      return result;
    } catch (error) {
      throw new HttpException({ reason: error }, HttpStatus.BAD_REQUEST);
    }
